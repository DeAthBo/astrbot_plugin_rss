...
- `/rss scheduler status`: 查看当前实例是否持有调度锁、任务数和锁归属进程。
- `/rss scheduler repair`: 尝试修复调度器并按当前订阅数据重建任务（若锁被其他存活进程持有，会给出提示）。
...
- **描述:** RSS 与图片抓取使用的 HTTP/HTTPS 代理服务器地址。
- **类型:** 字符串 (`string`)
- **提示:** 例如 `http://127.0.0.1:7890`；留空表示不使用插件级代理（仍可使用系统环境变量代理）。
- **默认值:** `""`（空字符串）
...
- **说明:** 该能力按平台类型判断（`aiocqhttp`），与平台 ID 是否为 `aiocqhttp` 无关；即使你自定义了平台 ID（如 `爱姆露`），也可以生效。
- **默认值:** `true`
...
A： 该问题已在 `v1.1.7` 进一步加固修复。除原有的任务重建与终止清理外，新增了调度锁机制，确保同一时间仅由一个实例执行定时任务。若出现异常，可先执行 `/rss scheduler status` 查看锁归属，再执行 `/rss scheduler repair` 重建任务；若提示锁被其他存活进程持有，请先停止重复运行的 AstrBot 进程。
